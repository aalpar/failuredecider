...
invariant has three levels. Each is necessary; only all three together are
sufficient.
...
\begin{quote}
At any point in time, the rate of resource consumption by all concurrent
recovery actions must remain below the rate of resource freed by the failures
they compensate for.
\end{quote}
...
per node produces ${\sim}7$~GiB of hints; at the default 1024~kbps throttle,
...
$\rho(\mathbf{G})$ grows. There exists a critical headroom $\mathbf{h}^*$ where
...
\begin{equation}
  \mathbf{G} = \begin{pmatrix} \varepsilon & \beta \\ \alpha & \varepsilon
  \end{pmatrix}
...
$\alpha\beta \geq 1$---the cross-resource loop (memory pressure $\to$ swap $\to$
disk saturation $\to$ blocked processes $\to$ memory pressure) has compound gain
$\geq 1$.
