...
We model recovery dynamics near the operating point as a first-order linearized
system. The state is the headroom vector $\mathbf{h}(t)$, and the feedback
structure is captured by the recovery gain matrix.
...
When headroom drops by $\Delta\mathbf{h}$, recovery demand changes by
$\mathbf{G} \cdot \Delta\mathbf{h}$. For $\mathbf{G} \geq 0$ this is an
increase (amplifying); for entries $G_{ij} < 0$ (e.g., backoff) it is a
decrease (stabilizing). In either case the iteration proceeds:
\begin{equation}
...
The linearization is locally valid when no eigenvalue of $\mathbf{G}$ lies on
the unit circle: $\rho(\mathbf{G}) < 1$ implies local asymptotic stability,
$\rho(\mathbf{G}) > 1$ implies local
instability.\footnote{This is the discrete Hartman-Grobman
