...
\paragraph{Local validity.} The discrete Hartman-Grobman
theorem~\cite{hartman1960lemma,grobman1959homeomorphisms,hartman1960local}
guarantees that the nonlinear system's local phase portrait is topologically
equivalent to the linearization when no eigenvalue of $\mathbf{G}$ lies on the
unit circle.
...
linearization predicts (a large perturbation can cross $\mathbf{h}^*$ even from
a locally stable operating point), and once past $\mathbf{h}^*$ it cannot
self-rescue. The linearized $\mathbf{h}^*$ is therefore a necessary condition
for stability, not a sufficient one: resistance to large perturbations requires
headroom margin beyond $\mathbf{h}^*$. The linear analysis never gives false
instability warnings---if $\rho(\mathbf{G}) \geq 1$, the system \emph{will}
cascade---but it can give false assurance of robustness when the basin of
attraction is smaller than the perturbation.
